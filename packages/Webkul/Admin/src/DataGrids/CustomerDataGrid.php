...
*/
        ...
        $this->addColumn([
            ...
            'closure'    => function ($row) {
                if ($row->status == 1) {
                    return '<span class="badge badge-md badge-success" style="cursor:pointer;" onClick="setAPILogout('.$row->customer_id.')" id="user'.$row->customer_id.'" >' . trans('admin::app.customers.customers.active') . '</span>';
                } else {
                    return '<span class="badge badge-md badge-danger">' . trans('admin::app.customers.customers.inactive') . '</span>';
                }
            },
