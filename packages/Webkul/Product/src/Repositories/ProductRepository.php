...
class ProductRepository extends Repository
{
    ...
    public function getAll($categoryId = null)
    {
       ...
       /* foreach($items as $k=>$v){
            $items[$k]["product_attributes"]= app('Webkul\Product\Helpers\View')->getAdditionalData($this->productRepository->findOrFail($v["id"]));
        }*/
     ...
     * @return \Illuminate\Support\Collection
     */
    public function getNewProducts($count=0)
    {
        if($count==0)$count = core()->getConfigData('catalog.products.homepage.no_of_new_product_homepage');
     ...
     * @return \Illuminate\Support\Collection
     */
    public function getFeaturedProducts($count=0)
    {
        if($count==0)$count = core()->getConfigData('catalog.products.homepage.no_of_featured_product_homepage');
    ...
    public function getSpecialsProducts($count=0)
    {
    ...
    public function getBestSellingProducts($count=0)
    {
        //SELECT `product_id`,SUM(`qty_ordered`) AS qty_ordered FROM `order_items` WHERE 1 GROUP BY product_id HAVING qty_ordered >0 ORDER BY qty_ordered DESC 
        $results = app(ProductFlatRepository::class)->scopeQuery(function ($query) {
            $channel = core()->getRequestedChannelCode();

            $locale = core()->getRequestedLocaleCode();

          // return  DB::select("SELECT pf.*, (SELECT SUM(`qty_ordered`) as qty_ordered FROM `order_items` WHERE pf.product_id=order_items.product_id GROUP BY product_id HAVING qty_ordered >0) as qty_ordered FROM `product_flat` pf WHERE 1 HAVING qty_ordered >0 ORDER BY qty_ordered DESC");
         return  $query->distinct()
            ->selectRaw('product_flat.*, (SELECT SUM(`qty_ordered`) as qty_ordered FROM `order_items` WHERE product_flat.product_id=order_items.product_id GROUP BY product_id HAVING qty_ordered >0) as qty_ordered')
            ->where('product_flat.status', 1)
            ->where('product_flat.visible_individually', 1)
            ->where('product_flat.channel', $channel)
            ->where('product_flat.locale', $locale)
        ...
        ;
          /*return $query->distinct()
                ...
                ->where('product_flat.channel', $channel)
                ->where('product_flat.locale', $locale)
                ->rightJoin('order_items', 'product_flat.product_id', '=', 'order_items.product_id');
              */
